...
![PM2](https://img.shields.io/badge/PM2-daemon-blue)
![SQLite](https://img.shields.io/badge/SQLite-WAL-orange)
![QQ Bot](https://img.shields.io/badge/QQ%20Bot-official%20gateway-ff69b4)
![Uptime](https://img.shields.io/badge/Uptime-7%C3%9724-brightgreen)
...
| 🔍 向量检索 | 独立向量服务 p3-vector，按用户隔离的 topK 语义召回，异步不阻塞主链 |
| 🌐 Web 客户端 | 零 CDN 依赖单页应用，SSE 逐字流式输出，展示响应插件来源；双层限流（个人 10 次/天 + 全站 100 次/天） |
| 🛡️ 云端自持 | PM2 守护 + 独立 watchdog + 健康探针 + 告警通道 + 日志轮转 + 6 小时定时备份 + 优雅退出 |
| 📊 可观测性 | 统一 metrics 采集，Prometheus 风格探针暴露 /health /ready /state /metrics |
...
~~~text
   ...
   (Web 插件)                          │
                         ...
                         │ p3-vector       向量检索服务    │
                         └──────────────────────────────┘
   自持层: PM2 守护 + watchdog + 健康探针 + alert 告警
...
~~~text
maidbot/
├── index.js                 # 主入口：事件分发 + 共享管道注册 + 优雅退出 + 定时备份
├── core/
...
│   ├── health.js            # 健康探针（/health /ready /state /metrics）
...
- **向量检索**：p3-vector 独立进程，按用户隔离 topK 语义召回，异步执行不阻塞回复主链
...
- **部署形态**：内置 Express 服务，可经 Nginx 反代对外暴露（流式场景建议关闭代理缓冲）
- **流式体验**：SSE 逐字输出，响应结束后展示 ⚙️ 由插件「xxx」响应
- **双层限流**（内存态，按自然日重置）：

| 维度 | 额度 | 超限行为 |
|------|------|----------|
| 个人（按真实 IP） | 10 次/天 | 429 + 前端锁定输入 |
| 全站（所有 Web 访客共享） | 100 次/天 | 429 + 页脚公示剩余量 |

- **状态接口**：实时公示全站剩余配额，页脚同步展示，便于监控
...
| P1 健康探针与告警 | 挂了第一时间知道 | core/health.js（/health /ready /state /metrics）；core/alert.js info/critical 双级告警 |
| P2 可观测性 | 量化系统表现 | metrics.js 统一采集（消息量 / C2C·GROUP 分类 / 重启次数）；Prometheus 风格 /metrics；Web 端状态面 |
| P3 向量检索 | 记得住聊过什么 | p3-vector 独立向量服务；用户隔离语义召回；异步不阻塞 |
| P4 记忆摘要与分层调度 | 长期记忆 | 双库联动 Worker：30s 同步 + 1800s 摘要；短期/长期分层沉淀 |
| P5 性能与体验 | 快且稳 | SSE 流式输出；LLM 超时 + 重试；双层限流保护资源；Nginx 关闭缓冲打通流式 |

---

## 🩺 可观测性与系统自持

- **健康探针**：提供 /health /ready /state /metrics 端点，便于集成 Prometheus / Grafana
...
- **进程编排**：PM2 管理 maidbot / p3-vector / watchdog 三进程，pm2 status 与 pm2 logs 日常巡检
...
~~~bash
git clone https://github.com/diandian1hao/maidbot.git && cd maidbot
npm install
